<template>
  <div class="trader-products">
    ...
    <div class="market-day-section">
      <div class="market-day-card">
        <h2>📅 Prochain jour de marché</h2>
        <div class="market-day-form">
          <input
            v-model="nextMarketDay"
            type="date"
            class="date-input"
            :disabled="savingMarketDay"
          />
          <button 
            @click="saveMarketDay" 
            class="btn-primary"
            :disabled="savingMarketDay || !nextMarketDay"
          >
            {{ savingMarketDay ? 'Enregistrement...' : 'Enregistrer' }}
          </button>
        </div>
        <p v-if="nextMarketDay" class="market-day-info">
          Prochain marché : <strong>{{ formatDate(nextMarketDay) }}</strong>
        </p>
      </div>
    </div>
    ...
    <div class="products-section">
      ...
      <div v-else class="products-grid">
        ...
        >
          ...
          <div v-if="product.data.stock_quantity !== null && product.data.stock_quantity !== undefined" class="product-stock">
      ...
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="saveProduct" class="product-form">
          ...
          <div class="form-group">
            ...
            <input
              v-model="productForm.next_market_day"
              type="date"
              :disabled="saving"
            />
...
const loadData = async () => {
  ...
  try {
    ...
    // Load vendor info to get next_market_day
    if (isOnline()) {
      const { data: vendor } = await supabase
        .from('vendors')
        .select('next_market_day')
        .eq('id', vendorId)
        .single()

      if (vendor?.next_market_day) {
        nextMarketDay.value = vendor.next_market_day
      }
    }
...
const saveMarketDay = async () => {
  if (!nextMarketDay.value || !traderVendorId.value) return

  savingMarketDay.value = true

  try {
    if (!isOnline()) {
      alert('Vous devez être en ligne pour enregistrer le jour de marché')
      savingMarketDay.value = false
      return
    }

    const { error } = await supabase
      .from('vendors')
      .update({ next_market_day: nextMarketDay.value })
      .eq('id', traderVendorId.value)

    if (error) throw error

    await syncService.syncVendors()
    alert('Prochain jour de marché enregistré !')
  } catch (error: any) {
    console.error('Error saving market day:', error)
